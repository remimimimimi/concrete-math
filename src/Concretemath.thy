...
subsection "Essential Metamath syntax"

(* TODO: Make subsets of allowed string characters for LABEL and MATH_SYMBOL *)
type_synonym label = string
type_synonym math_symbol = string

type_synonym const = math_symbol
type_synonym var = math_symbol

type_synonym typecode = const

datatype declare_kind = Variables | Constants

datatype axiom_kind = Essential | Normal

datatype proof_kind = Normal | Compressed

datatype statement = Scope "statement list"
...
definition empty_db :: database where
  "empty_db = \<lparr>
    constants = {},
    vars = {},
    disj = {},
...
definition declared_constant :: "database \<Rightarrow> const \<Rightarrow> bool" where
  "declared_constant db c = (c \<in> constants db)"

definition declared_var :: "database \<Rightarrow> var \<Rightarrow> bool" where
  "declared_var db v = (v \<in> vars db)"

definition disjoint_vars :: "database \<Rightarrow> var \<Rightarrow> var \<Rightarrow> bool" where
  "disjoint_vars db v1 v2 = (\<exists>d \<in> disj db. v1 \<in> d \<and> v2 \<in> d)"

(* Lookup active floating hypothesis *)
definition lookup_afh :: "database \<Rightarrow> var \<Rightarrow> label option" where
  "lookup_afh db v = flab db v"

(* Lookup earliest essential hypothesis *)
definition lookup_eeh :: "database \<Rightarrow> math_symbol list \<Rightarrow> label option" where
  "lookup_eeh db stmt = elab db stmt"

definition add_constant :: "database \<Rightarrow> const \<Rightarrow> database option" where
  "add_constant db c =
    (if declared_constant db c \<or> declared_var db c
         then None
         else Some (db \<lparr>constants := constants db \<union> {c} \<rparr>))"

definition add_var :: "database \<Rightarrow> var \<Rightarrow> database option" where
  "add_var db v =
    (if declared_constant db v \<or> declared_var db v
         then None
         else Some (db \<lparr>vars := vars db \<union> {v} \<rparr>))"

definition add_disjoint_vars :: "database \<Rightarrow> var set \<Rightarrow> database" where
  "add_disjoint_vars db vs = db \<lparr>
    disj := disj db \<union> {vs}
  \<rparr>"

definition add_floating_hypothesis :: "database \<Rightarrow> const \<Rightarrow> var \<Rightarrow> label \<Rightarrow> database option" where
...
definition add_essential_hypothesis :: "database \<Rightarrow> math_symbol list \<Rightarrow> label \<Rightarrow> database" where
...
definition find_vars :: "database \<Rightarrow> math_symbol list \<Rightarrow> var set" where
...
definition make_assertion :: "database \<Rightarrow> math_symbol list \<Rightarrow> assertion" where
...
primrec substitute :: "math_symbol list \<Rightarrow> (var \<rightharpoonup> (math_symbol list)) \<Rightarrow> math_symbol list" where
  "substitute [] subst = []" |
